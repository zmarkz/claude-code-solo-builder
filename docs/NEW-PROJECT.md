...
├── commands/            ← 14 slash commands (incl. /orchestrate-loops)
└── settings.json        ← deny/ask/allow permissions perimeter

scripts/
├── session-start.sh     ← SessionStart hook
├── guard-dangerous-command.sh  ← PreToolUse Bash hook
├── guard-file-domain.sh        ← PreToolUse Write/Edit hook (no-op unless a durable leaf sets LEAF_DOMAIN_GLOBS)
...
├── design-lint.sh              ← PostToolUse design lint
├── tasks-sync.sh               ← TASKS.md → tasks.json (Mode 3 durable-loop substrate)
└── session-snapshot.sh         ← PreCompact hook

tasks.json               ← machine-readable task queue (generated by tasks-sync.sh)
Makefile
docker-compose.yml
